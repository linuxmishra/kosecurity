...
if ( ! function_exists( 'jobscout_customizer_theme_info' ) ) :
	...
	function jobscout_customizer_theme_info( $wp_customize ) {
		...
		$theme_info .= sprintf( '%1$s<a href="%2$s" target="_blank">%3$s</a>', esc_html__( 'Demo Link : ', 'jobscout' ), esc_url( __( 'https://demo.raratheme.com/jobscout/', 'jobscout' ) ), esc_html__( 'Click here.', 'jobscout' ) );

	    $theme_info .= '</p><p>';

	    /* translators: 1: string, 2: documentation url, 3: string */
	    $theme_info .= sprintf( '%1$s<a href="%2$s" target="_blank">%3$s</a>', esc_html__( 'Documentation Link : ', 'jobscout' ), esc_url( 'https://raratheme.com/documentation/jobscout/' ), esc_html__( 'Click here.', 'jobscout' ) );
	...
	$manager->add_section(
		new JobScout_Customize_Section_Pro(
			...
			array(
				...
				'pro_url'  => 'https://raratheme.com/wordpress-themes/jobscout-pro/'
			)
		)
	);
}